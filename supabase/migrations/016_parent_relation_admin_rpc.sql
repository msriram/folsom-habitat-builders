...
begin
  ...
  if not exists (select 1 from profiles where id=target_parent and team_id=current_team_id() and role in ('parent','coach') and approval_status='approved' and is_active) then raise exception 'approved parent or coach not found'; end if;
