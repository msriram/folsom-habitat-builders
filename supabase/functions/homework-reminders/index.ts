...
const siteUrl = Deno.env.get("PUBLIC_SITE_URL") || "https://msriram.github.io/folsom-habitat-builders";
...
Deno.serve(async (req) => {
  ...
  const {data:reminders,error} = await admin.from("homework_reminders")
    .select("id,assignment_id,student_id,recipient_id,reminder_kind,attempts,scheduled_for,assignments(id,title,description,due_at,week_number,reminder_highlight),student:profiles!homework_reminders_student_id_fkey(display_name),recipient:profiles!homework_reminders_recipient_id_fkey(display_name,email)")
  ...
  for (const reminder of reminders || []) {
    ...
    const worksheetLink = homeworkSafe
      ? `<p style="margin:18px 0"><strong>Core Values question · ${coreValue} ${coreActivity}</strong><br><span style="color:#53645a">${coreQuestion}</span></p>`
