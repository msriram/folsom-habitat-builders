...
Deno.serve(async req=>{
  ...
  let subject="Habitat Builders homework reminders are connected";
  let content=`<h2>Gmail is connected</h2><p>This is a test message from the Habitat Builders Team Room.</p><p>Weekly homework reminders will be sent here from this Gmail account once the schedule is enabled.</p>`;
  if(body?.kind==="week2" || body?.kind==="current"){
    let assignmentQuery=admin.from("assignments").select("id,title,description,due_at,week_number,published,reminder_highlight").order("due_at");
    ...
    const highlightItems=String(assignment.reminder_highlight||"").split(/\r?\n/).map(item=>item.trim().replace(/^(?:[-*•]|\d+[.)])\s*/,"")).filter(Boolean);
    const highlightBlock=highlightItems.length?`<section style="background:#fff8df;border-left:4px solid #d69b19;padding:14px 16px;margin:18px 0"><p style="margin:0 0 8px;font-size:12px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#6b5314">This week's focus</p><ol style="margin:0;padding-left:20px">${highlightItems.map(item=>`<li style="margin:8px 0">${esc(item)}</li>`).join("")}</ol></section>`:"";
    ...
    const isReminder=body?.reminder===true&&assignment.published===true;
    subject=`${isReminder?'Reminder: ':'Habitat Builders: '}Week ${assignment.week_number} Homework${isReminder?'':' Posted'}`;
    content=`<p style="margin:0;color:#64746d;font-size:12px;letter-spacing:.1em;text-transform:uppercase">Habitat Builders · Week ${assignment.week_number}${isReminder?' · Reminder':''}</p><h1 style="font-size:25px">${isReminder?'REMINDER: ':''}TOPIC: ${esc(assignment.title)}</h1>${highlightBlock}<p>Hello Sriram,</p><p>${isReminder?'This is a reminder to complete the homework below before the due date.':'Please help your student set aside time to explore, create their own response, and upload any required work.'}</p><p><strong>Due:</strong> ${esc(due)}</p><div style="background:#f2f7ef;border-left:4px solid #175b3c;padding:14px 16px">${esc(assignment.description)}</div><h2 style="font-size:18px">What to complete</h2><ol>${(questions||[]).map(q=>`<li style="margin:8px 0">${esc(q.prompt)}</li>`).join("")}</ol>${robotTask?`<h2 style="font-size:18px">Robot programming</h2><p><strong>${esc(robotTask.title)}</strong><br>${esc(robotTask.description)}</p><p>Complete the program, upload a screenshot of the finished work, and explain in a few sentences how the program worked.</p><p><a href="${esc(robotTask.cs2n_url)}">Open the programming activity</a></p>`:""}<p><a href="https://msriram.github.io/folsom-habitat-builders/portal.html?tab=homework" style="display:inline-block;background:#175b3c;color:#fff;padding:11px 16px;border-radius:6px;text-decoration:none">Open homework</a></p>`;
  }
