...
const fallbackAssignments = {
  ...
  3: { title: 'Project Sparks and Challenge Story', due: 'Wednesday, August 26', priority: 'Choose a biodiversity problem, answer the project questions, and complete the first Core Values reflection.' },
...
function applyRhythm() {
  ...
  // Discovery Activity 1 comes from the third page of the Core Values book.
  // Keep the coach card, student task list, form, and stored question aligned.
  if (week3Detail) {
    const taskList = week3Detail.querySelector('.cell-prompt ol');
    const discoveryTask = taskList?.lastElementChild;
    if (discoveryTask) discoveryTask.innerHTML = '<strong>Core Values · Discovery Activity 1:</strong> Find out how many countries have FIRST LEGO League teams. Choose at least three countries and learn how to say “hello” and “My name is...” in languages spoken there.';
    const discoveryField = week3Detail.querySelector('[data-core-values-response]');
    if (discoveryField) discoveryField.innerHTML = 'Core Values · Discovery Activity 1 — FIRST LEGO League around the world<textarea name="core_values_discovery" rows="6" minlength="40" maxlength="2000" required></textarea>';
  }
