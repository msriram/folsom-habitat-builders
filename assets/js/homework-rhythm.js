...
const fallbackAssignments = {
  ...
  2: { title: 'Team name decision: Rainforest Rangers', due: 'Wednesday, August 19', priority: 'Review the original proposals, the Rainforest Rangers decision, and the biodiversity questions the name can help us investigate.' },
...
// For Sessions 5-8, the homework is the design brief that comes before Friday
// practice. The session page then builds, tests, records, and decides the next
// iteration rather than asking students to retroactively describe a test.
Object.assign(notebookHomework, {
  5: { session: 5, reflects: 4, title: 'Model build inspiration and project curiosity', link: 'meeting-05.html', questions: [['session4_observation', 'Session 4 was an early build-and-test session. What did your group try, measure, or notice? Name one thing that still needs work; it is okay if nothing is working reliably yet.'], ['build_contribution', 'For Session 5, which remaining build could you help with: the M8/M9 tree house, M13 or M15, or a base for M13–M15? What small job, part, or reset check could you take responsibility for?'], ['innovation_question', 'Ask one real question about a local biodiversity problem the Rainforest Rangers could investigate. Why does this question matter to people, animals, plants, or a habitat?'], ['inspiration_to_share', 'Watch the SPIKE build reference and one inspiration video below. What one idea could the team adapt—not copy—for a base robot or attachment? It could be a sturdy mount, simple lift, guide, pusher, grabber, or quick-change connection. Explain how it might help our current models.']] },
  6: { session: 6, reflects: 5, title: 'Rainforest Rangers ideas and first attachment drawing', link: 'meeting-06.html', questions: [['session5_carry_over', 'Session 5 was an early attachment-and-measurement day. What is one robot, model, or field-setup item that still needs to be finished before we can test seriously?'], ['first_attachment_drawing', 'Draw your first attachment idea for a model or robot action. Label what it would push, lift, guide, pull, or carry and where it would connect to the base robot. Upload a photo of the drawing if you can.'], ['rainforest_rangers_idea', 'Use the shared Rainforest Rangers Google Doc. Choose a rainforest habitat and explain one Protect–Rebuild–Educate idea: what needs protection, what could be rebuilt, or what should people learn?'], ['google_doc_finished', 'After you add your thorough Rainforest Rangers write-up to the shared Google Doc, check the completion box below.']] },
  7: { session: 7, reflects: 6, title: 'Attachment revision plan', link: 'meeting-07.html', questions: [['revision_target', 'Choose one result from the last robot test that needs improvement. What happened, and what is the specific failure you want to prevent?'], ['revision_design', 'Draw or describe one attachment revision. Label what changes, what stays the same, and why this version should work better.'], ['comparison_plan', 'Plan a fair comparison: how will the team keep the same home base, start position, model reset, and program while testing the old and new attachment?'], ['decision_rule', 'What result will make the team keep the new attachment, revise it again, or stop working on this model for now?']] },
  8: { session: 8, reflects: 7, title: 'Integrated attachment run plan', link: 'meeting-08.html', questions: [['run_sequence_plan', 'Choose one attachment action to add to a short robot run. Write the sequence from home base through the action and back to a safe stop or return.'], ['reset_plan', 'List what must be reset before every test: robot start position, attachment position, model position, and any program setting.'], ['reliability_goal', 'Set a repeatability goal for Session 8. How many successful runs out of ten will show that this action is ready to keep?'], ['next_iteration_question', 'What one question should the team answer from the Session 8 test record before designing the next attachment or route?']] }
});
...
notebookHomework[4] = {
  ...
  questions: [
    ...
    ['spike_build_guide', 'Read the SPIKE Prime building instructions. In your own words, name two build choices that make a robot sturdy and repeatable. Which one should Rainforest Rangers use first, and why?']
  ]
};
...
const futureTaskTitles = {
  attachment_plan: 'Attachment plan', wireframe_route: 'Wireframe route', wireframe_measurements: 'Measurement plan', spike_build_guide: 'SPIKE robot build guide', attachment_target: 'Attachment target', attachment_sketch: 'Attachment sketch', route_plan: 'Route plan', five_run_test_plan: 'Five-run test plan', mission_action_plan: 'Mission action plan', attachment_change_plan: 'Attachment change plan', measurement_check: 'Measurement check', test_record_plan: 'Test record plan', revision_target: 'Revision target', revision_design: 'Revision design', comparison_plan: 'Fair comparison plan', decision_rule: 'Decision rule', run_sequence_plan: 'Run sequence', reset_plan: 'Reset plan', reliability_goal: 'Repeatability goal', next_iteration_question: 'Next iteration question',
  ...
  session4_observation: 'Session 4 observation', build_contribution: 'Build contribution', innovation_question: 'Innovation Project question', inspiration_to_share: 'Build inspiration to share',
  session5_carry_over: 'What is still unfinished?', first_attachment_drawing: 'Draw the first attachment idea', rainforest_rangers_idea: 'Rainforest Rangers idea', google_doc_finished: 'Google Doc write-up complete',
...
function applyRhythm() {
  ...
  if (week2Detail && week2Detail.dataset.homeworkOnly !== 'team-name') {
    ...
    week2Detail.innerHTML = `<summary class="notebook-title"><div><span>Week 2 · Team name decision</span><h3>How we chose Rainforest Rangers</h3></div><strong>Due Wednesday, August 19</strong><em>Click to expand</em></summary><section class="notebook-cell"><div class="cell-prompt"><span>Homework questions</span><h4>Connect our interests to a shared team direction</h4><p>The original assignment collected each student’s idea. Rainforest Rangers is now the team’s selected name, and these responses preserve how the decision was made.</p><ol><li>What team name did you propose?</li><li>What biodiversity cause should this name help us investigate?</li><li>Why does this name fit our interests and BIOGLOW?</li><li>What should the team investigate or build next?</li></ol></div></section><footer><span>Connected schedule</span><span><a href="meeting-02.html">Session 2 plan</a> · <a href="resources.html">Official resources</a></span></footer><section class="submission-gate" data-homework-gate="2"><h3>Turn in Week 2</h3><p>Sign in with an approved student account to submit this homework.</p><a class="button primary" href="login.html">Sign in</a></section><form class="homework-submit notebook-response" data-homework-form data-week-number="2" hidden><div class="section-title"><div><span class="eyebrow">Your response</span><h3>Week 2 submission</h3></div><span class="status-chip" data-submit-status>Not submitted</span></div><label>What team name did you propose?<input name="team_name" maxlength="120" required></label><label>What biodiversity cause should this name help us investigate?<textarea name="cause" rows="5" minlength="40" maxlength="2000" required></textarea></label><label>Why does this name fit our interests and BIOGLOW?<textarea name="reason" rows="5" minlength="40" maxlength="2000" required></textarea></label><label>What should the team investigate or build next?<textarea name="next_step" rows="4" minlength="20" maxlength="1200" required></textarea></label><div data-existing-files class="submission-files"></div><button class="button primary" type="submit">Submit Week 2 homework</button><p data-homework-message aria-live="polite"></p></form>`;
  }
  ...
  if (week3Detail) {
    const sessionQuestions = [
      ...
      ['model_purpose', 'Mission name and Spark connection — our team name is Rainforest Rangers. Choose an interest angle that matters to you, such as invasive species, rainforest protection, or another biodiversity idea. How could Rainforest Rangers investigate or build something that helps?']
    ];
    ...
    if (weekHeading) weekHeading.textContent = 'Model build and Rainforest Rangers connection';
    if (promptHeading) promptHeading.textContent = 'Use what we did in Session 2';
    if (promptText) promptText.textContent = 'Think back to the Session 2 models and connect your own biodiversity interest to our Rainforest Rangers team name. Field measurements move to next week.';
  ...
  Object.entries(notebookHomework).forEach(([weekNumber, assignment]) => {
    ...
    const fields = assignment.questions.map(([key, prompt], index) => key === 'google_doc_finished'
      ? `<label class="homework-complete-check"><span>Task ${index + 1} · ${futureTaskTitle(key)}</span><input name="${key}" type="checkbox"> I added my rainforest habitat research and Protect–Rebuild–Educate idea to the shared Google Doc.</label>`
      : `<label><span>Task ${index + 1} · ${futureTaskTitle(key)}</span>${prompt}<textarea name="${key}" rows="4" minlength="20" maxlength="2000" required></textarea></label>`).join('');
    ...
    const weekFiveResources = Number(weekNumber) === 5 ? `<section class="notebook-cell resource-cell"><div class="cell-prompt"><span>Watch and borrow an idea</span><h4>Attachment and base-robot inspiration</h4><p>These examples are for noticing useful design choices—not for copying a complete robot. Look for a sturdy connection, a simple motion, or an easy reset that could help the team finish and test its own models.</p></div><div class="worksheet-preview-grid"><a class="worksheet-preview-card" href="https://education.lego.com/en-us/product-resources/spike-prime/downloads/building-instructions/" target="_blank" rel="noopener"><img src="https://assets.education.lego.com/v3/assets/blt293eea581807678a/blt25699216b86fe3af/5ec6d1e87976043edd42adb4/dozer-blade.png?locale=en-us&amp;auto=webp&amp;format=jpeg&amp;width=800&amp;quality=90&amp;fit=bounds" alt="LEGO Education SPIKE Prime dozer blade build example" loading="lazy"><strong>Official SPIKE tools and driving bases ↗</strong><small>Look at the Advanced Driving Base, Dozer Blade, and Lift Arm build instructions.</small></a><a class="worksheet-preview-card" href="https://www.youtube.com/watch?v=dGwxFroBn-s" target="_blank" rel="noopener"><img src="https://i.ytimg.com/vi/dGwxFroBn-s/hqdefault.jpg" alt="MiniLITE SPIKE Prime FLL base robot video preview" loading="lazy"><strong>Watch: MiniLITE base robot ↗</strong><small>Notice how the base leaves room for an attachment motor and quick, sturdy connections.</small></a><a class="worksheet-preview-card" href="https://www.youtube.com/watch?v=d3txcEZVfQA" target="_blank" rel="noopener"><img src="https://i.ytimg.com/vi/d3txcEZVfQA/hqdefault.jpg" alt="MiniMAX SPIKE Prime FLL base robot video preview" loading="lazy"><strong>Optional: MiniMAX build tour ↗</strong><small>Compare how a larger base can make space for more than one attachment motion.</small></a></div></section>` : '';
    const weekSixResources = Number(weekNumber) === 6 ? `<section class="notebook-cell resource-cell"><div class="cell-prompt"><span>Shared project research</span><h4>Rainforest Rangers · Protect · Rebuild · Educate</h4><p>Use the shared document to research one rainforest habitat: where it is, what grows there, who lives there, what is in danger, what is causing the problem, and how people can protect, rebuild, or educate. Use the Team Room Ask AI helper to research—but write your own explanation in the Google Doc.</p></div><div class="notebook-downloads"><a href="https://docs.google.com/document/d/1aggLDJPzxi8gMGfWtsdwpKp7Xu1iM58yEGxzZ6L27l0/edit?pli=1&amp;tab=t.0" target="_blank" rel="noopener"><strong>Open shared Rainforest Rangers ideas Doc ↗</strong><small>Add your research and idea there</small></a><a href="portal.html?tab=guide" target="_blank" rel="noopener"><strong>Open Ask AI ↗</strong><small>Use it to investigate your habitat questions</small></a></div></section>` : '';
    const planCopy = attachmentPlanning
      ...
      : Number(weekNumber) === 5
        ? 'Session 4 was mostly an early try-out: the team explored attachments, test-ran a few ideas, and took some measurements. This week circles back to the Innovation Project while preparing each student to help finish the remaining models and continue testing in Session 5.'
      : Number(weekNumber) === 6
        ? 'Session 5 was an early exploration day, not a formal attachment-test session. This homework prepares the team to finish the robot and field setup, review a first attachment drawing, and build the Rainforest Rangers project direction together.'
