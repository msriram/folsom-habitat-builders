...
function renderCoachWeekTwoCard() {
  ...
  detail.innerHTML = `<summary class="notebook-title"><div><span>Week 2 · Coach decision record</span><h3>How the team chose Rainforest Rangers</h3></div><strong>Due Wednesday, August 19</strong><em>Click to expand</em></summary><section class="notebook-cell"><div class="cell-prompt"><span>Coach context</span><h4>Team identity and project direction</h4><p>Student responses capture the proposals that informed the decision. Rainforest Rangers is the selected team name; retain the original answers as evidence of the team’s process.</p></div></section><section class="notebook-cell"><div class="cell-prompt"><span>Answer 1</span><h4>Team name proposal</h4><p>What team name did the student propose?</p></div></section><section class="notebook-cell"><div class="cell-prompt"><span>Answer 2</span><h4>Biodiversity cause</h4><p>What biodiversity cause should that name help the team investigate?</p></div></section><section class="notebook-cell"><div class="cell-prompt"><span>Answer 3</span><h4>Why the name fits</h4><p>Why does the name fit the team’s interests and BIOGLOW?</p></div></section><section class="notebook-cell"><div class="cell-prompt"><span>Answer 4</span><h4>Next team step</h4><p>What should the team investigate or build next?</p></div></section><footer><span>Connected schedule</span><span><a href="meeting-02.html">Session 2 plan</a> · <a href="resources.html">Official resources</a></span></footer><section class="submission-gate"><h3>Review student responses</h3><p>Open the review workspace to read every student response and leave coach feedback.</p><a class="button primary" href="admin-homework.html">Open homework review</a></section>`;
}
...
function renderCoachWeekThreeCard() {
  ...
  if (!detail || detail.dataset.coachPresentation === 'true') return;
  detail.dataset.coachPresentation = 'true';
  detail.innerHTML = `<summary class="notebook-title"><div><span>Week 3 · Coach homework plan</span><h3>Model build and Rainforest Rangers connection</h3></div><strong>Due Wednesday, August 26</strong><em>Click to expand</em></summary>
    <section class="notebook-cell"><div class="cell-prompt"><span>Coach context</span><h4>Reflect on Session 2 work</h4><p>Students look back at one model and connect their own biodiversity interest to the Rainforest Rangers team name. Field measurement work moves to Week 4.</p></div></section>
    ...
    <section class="notebook-cell"><div class="cell-prompt"><span>Task 3</span><h4>Model build and attachment idea</h4><p>What model did the student build, improve, or observe in Session 2? After rewatching the Robot Game Missions video, can they explain how it works and what kind of attachment could help a robot complete it? They may draw the idea and should be ready to explain it in class.</p><div class="notebook-downloads"><a href="https://www.youtube.com/watch?v=uhZZ8O1StiQ" target="_blank" rel="noopener"><strong>Rewatch Robot Game Missions video ↗</strong><small>Observe how the mission models work</small></a></div></div></section>
    <section class="notebook-cell"><div class="cell-prompt"><span>Task 4</span><h4>Mission name and Spark connection</h4><p>Our team name is Rainforest Rangers. Which interest angle—such as invasive species, rainforest protection, or another biodiversity idea—matters to the student? How could Rainforest Rangers investigate or build something that helps?</p></div></section>
    <section class="notebook-cell"><div class="cell-prompt"><span>Task 5</span><h4>Core Values · Discovery Activity 1</h4><p>FIRST LEGO League is done all over the world. Find out how many countries have FIRST LEGO League teams. Choose at least three of those countries and learn how to say “hello” and “My name is...” in languages spoken there.</p><div class="notebook-downloads"><a href="downloads/bioglow/core-values-discovery-1.pdf#page=3" target="_blank" rel="noopener"><strong>Discovery Activity 1 · page 3 ↗</strong><small>Open the Core Values activity book</small></a></div></div></section>
...
async function openStudentForm(db, studentId, form, gate, week) {
  ...
  if (submission) {
    ...
    for (const field of form.querySelectorAll('[name]')) {
      ...
      if (field.type === 'checkbox') field.checked = Boolean(answers[field.name]);
      else field.value = answers[field.name] || '';
    }
  ...
  form.onsubmit = async event => {
    ...
    const answers = [...form.querySelectorAll('[name]')].filter(field => !['files', 'cs2n_screenshot'].includes(field.name)).map((field, index) => ({ submission_id: saved.id, question_key: field.name, display_order: index, answer_type: field.tagName === 'TEXTAREA' ? 'long_text' : 'text', answer_text: field.type === 'checkbox' ? (field.checked ? 'Completed' : '') : field.value }));
